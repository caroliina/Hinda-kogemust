...
<div class = "header">
	...
	<div class="add">
		<form action="" method="POST">
			<input name="add" type="submit" class="button" value="Lisa asutus"></form>
	</div>
...
while($row = mysql_fetch_array($result)) {
  ...
  echo '<form action="admin.php" id="' . $row['place_id'] . '" method="POST"><input name="choose_place" type="submit" class="button" value="' . $row['name'] . '"></form>';
}
?>
...
function createTableMain() {
	echo '<table style="width:1000px"><tr class="main"><td>Nimi</td><td><form action="admin.php" method="POST"><input name="sort_name_desc" type="submit" class="button" value="↑" '. $sort_dis .'></form>';
	echo '<form action="admin.php" method="POST"><input name="sort_name_asc" type="submit" class="button" value="↓" '. $sort_dis .'></form></td>';

	echo '<td>Asukoht</td><td><form action="admin.php" method="POST"><input name="sort_location_desc" type="submit" class="button" value="↑" '. $sort_dis .'></form>';
	echo '<form action="admin.php" method="POST"><input name="sort_location_asc" type="submit" class="button" value="↓" '. $sort_dis .'></form></td>';

	echo '<td>Keskmine hinne</td><td><form action="admin.php" method="POST"><input name="sort_average_desc" type="submit" class="button" value="↑" '. $sort_dis .'></form>';
	echo '<form action="admin.php" method="POST"><input name="sort_average_asc" type="submit" class="button" value="↓" '. $sort_dis .'></form></td>';

	echo '<td>Hinnatud (korda)</td><td><form action="admin.php" method="POST"><input name="sort_ratings_desc" type="submit" class="button" value="↑" '. $sort_dis .'></form>';
	echo '<form action="admin.php" method="POST"><input name="sort_ratings_asc" type="submit" class="button" value="↓" '. $sort_dis .'></form></td>';

	echo '<td>Muuda/Kustuta</td></tr>';
}
...
function showPlaces() {
	...
	while($q_row = mysql_fetch_array($query)) {
		...
		$rate_btn = '<td></td><td style="display: flex;"><form action="" method="POST"><input type="hidden" value="' . $q_row['location'] . '" name="location"><input type="hidden" value="' . $q_row['name'] . '" name="business_name"><input name="edit" type="submit" class="button" value="Muuda"></form>';
		$delete_btn = '<form action="" method="POST"><button name="delete" type="submit" class="button" value="' . $q_row['name'] . '">[X]</button></td></tr>';

		echo $value . $rate_btn . $delete_btn;
		
	}
	echo '</table><div class="navigate">';
	echo '<form action="admin.php" method="POST"><input name="previous" type="submit" class="button" value="Eelmine" id="previous" '. $disabled .'>';
	echo '<form action="admin.php" method="POST"><input name="next" type="submit" class="button" value="Järgmine" '. $disabled_next .'></div> ';
}



if (isset($_POST["delete"])) {
        	$name_del = $_POST['delete'];
        	$query_del = mysql_query("DELETE FROM t123661_ratings WHERE name = '$name_del'");
        	$query_del = mysql_query("DELETE FROM t123661_details WHERE name = '$name_del'");
		}
